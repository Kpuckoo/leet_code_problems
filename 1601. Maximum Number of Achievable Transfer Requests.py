class Solution:
    def maximumRequests(self, n: int, requests: List[List[int]]) -> int:
        def cyclic(path):
            return requests[path[0]][0] == requests[path[-1]][1]

        def joins(path, e):
            return requests[e][0] == requests[path[-1]][1]

        def overlapping(c1, c2):
            return any([True for e in c1 if e in c2])

        def include(cycle, cycles):
            return [c for c in cycles if not overlapping(c, cycle)] + [cycle]
